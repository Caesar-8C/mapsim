	...
	def main(self):
		def activateElement(pos):
			...
			for node in list(self.G.nodes):
				dist = np.linalg.norm(np.subtract(self.G.nodes[node]['coordinates'], pos))
				if dist < self.G.degree[node]*10:
			...
			for edge in list(self.G.edges):
				...
				if between(x, x1, x2) and between(y, y1, y2):
					...
					if dist < self.G.edges[edge]['width']/2 or dist < 5:
		...
		def eventHandlerLoop():
			for event in pyg.event.get():
				...
				if event.type == pyg.MOUSEMOTION and not self.activeNode == False:
					self.G.nodes[self.activeNode]['coordinates'] = tupleSum(self.G.nodes[self.activeNode]['coordinates'], event.rel)
				if event.type == pyg.MOUSEMOTION and not self.activeEdge == False:
					self.G.nodes[self.activeEdge[0]]['coordinates'] = tupleSum(self.G.nodes[self.activeEdge[0]]['coordinates'], event.rel)
					self.G.nodes[self.activeEdge[1]]['coordinates'] = tupleSum(self.G.nodes[self.activeEdge[1]]['coordinates'], event.rel)
		...
		def drawRect(color, node, successor):
			...
			width = self.G.edges[node, successor]['width']/2
			x3 = x1 + np.cos(angle)*width
			y3 = y1 + np.sin(angle)*width

			x4 = x1 - np.cos(angle)*width
			y4 = y1 - np.sin(angle)*width

			x5 = x2 - np.cos(angle)*width
			y5 = y2 - np.sin(angle)*width

			x6 = x2 + np.cos(angle)*width
			y6 = y2 + np.sin(angle)*width

			pyg.draw.polygon(self.screen, color, ((x3, y3), (x4, y4), (x5, y5), (x6, y6)))
		def drawRooms(color, node, successor):
			...
			for room in self.G.edges[node, successor]['rooms']:
				...
				pyg.draw.line(self.screen, color, (x3, y3), (x4, y4), 3)
		def drawLanes(color, node, successor):
			...
			if lanes > 1:
				...
				for i in range(lanes-1):
					...
					pyg.draw.line(self.screen, color, (x3, y3), (x4, y4), 1)
		def drawAgents(color):
			...
			for agentIndex in self.agents:
				...
				pyg.draw.circle(self.screen, color, (x3, y3), int(laneWidth/2))
		...
		def drawSuccessors(node, visited):
			...
			# 	if node in edge and self.G.edges[edge]['width'] > rad:
			# 		rad = self.G.edges[edge]['width']
			# pyg.draw.circle(self.screen, (128, 128, 0), self.G.nodes[node]['coordinates'], int(rad/2.))
			pyg.draw.circle(self.screen, (0, 128, 0), self.G.nodes[node]['coordinates'], self.G.degree[node]*10)
